...
// Vite only injects VITE_/SITE_-prefixed vars into import.meta.env; it does
// not load .env into process.env for server-only secrets (SUPABASE_URL,
// SUPABASE_SECRET_KEY, ADMIN_PASSWORD, SESSION_SECRET, RESEND_API_KEY).
// process.loadEnvFile is native to Node 22+ (see package.json "engines"),
// so this needs no extra dependency. Only for local dev/preview — Vercel
// and any other real host inject env vars into process.env directly.
